...
if __name__ == '__main__':
    ...
    ok = True
    for v in vectors:
        c = Blowfish(bytes.fromhex(v[0]))
        e = c.encrypt(bytes.fromhex(v[1]))
        o = ''.join(format(x, '02X') for x in e)
        if o != v[2]:
            print("VECTOR ENCRYPT FAIL: expecting %s, got %s" % (repr(v), o))
            ok = False
    for v in vectors:
        c = Blowfish(bytes.fromhex(v[0]))
        e = c.decrypt(bytes.fromhex(v[2]))
        o = ''.join(format(x, '02X') for x in e)
        if o != v[1]:
            print("VECTOR DECRYPT FAIL: expecting %s, got %s" % (repr(v), o))
            ok = False
